...
WORKDIR /
# build asterisk
ADD asterisk-20.tar.gz ./
WORKDIR asterisk-18.20.0
RUN ./configure --with-pjproject-bundled --with-jansson-bundled --with-srtp=/usr/lib64 --libdir=/usr/lib64
...
WORKDIR /
ADD mysql-connector-odbc-8.2.0-linux-glibc2.28-x86-64bit.tar.gz ./
